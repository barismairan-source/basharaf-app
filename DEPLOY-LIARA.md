...
**خطای `28P01` (password authentication failed):** معمولاً به این معنی نیست
که پسورد در پنل اشتباه است — مشکل **پارس شدن connection string** است. اگر
پسورد auto-generated پنل Liara شامل کاراکترهای خاص (`@ # % & ...`) باشد و
بدون percent-encoding داخل `DATABASE_URL` قرار گرفته باشد، پارسر می‌تواند
host/user/pass را اشتباه جدا کند. از نسخه‌ی `v9.1.0` به بعد، `lib/db/client.ts`
خودش connection string را با یک پارسر مقاوم (که آخرین `@` قبل از host را پیدا
می‌کند و پسورد را جداگانه می‌خواند) می‌شکند، پس این مشکل دیگر نباید رخ دهد.
اگر باز هم `28P01` گرفتید: مطمئن شوید کل `DATABASE_URL` را عیناً (بدون فاصله
یا newline اضافه) از پنل کپی کرده‌اید و username آن دقیقاً `root` است.
