...
| | |
|---|---|
| **نسخه** | `0.9.10-order-board` |
| **آخرین به‌روزرسانی** | 2026-06-14 — اکانت: ۲ |
| **Build/tsc** | tsc سبز ✅ (۰ خطا) — `npm run build` ✅ سبز |
| **دیپلوی** | 🆕 `v0.9.10-order-board/basharaf-deploy.zip` + `v0.9.10-order-board/db-ordering-board-realtime-migration.sql` آماده‌ی دیپلوی (باکس۳: `/orders` پنل عملیاتی پرسنل — Kanban + state machine + Realtime + چاپ؛ **migration جدید [orders/order_lines/order_events → supabase_realtime] هنوز روی DB اجرا نشده** — بدون آن همه‌چیز کار می‌کند ولی هشدار صوتی سفارش جدید زنده نیست). ⚠️ `v0.9.9-order-checkout/db-ordering-checkout-migration.sql` (باکس۲) هم **هنوز اجرا نشده** — مستقل و قدیمی‌تر، همچنان بلاک‌کننده‌ی `/order/checkout`. ✅ `v0.9.8-order-public` (باکس۰+۱) دیپلوی شده. ✅ `v0.9.7-menu-channel-public/basharaf-deploy.zip` (push شده). zipهای قبلی هنوز دیپلوی نشده: `basharaf-tasks-ops-liara.zip` (شامل `db-operations-migration.sql`)، و `basharaf-0.9.4-inv-foundation.zip` + `db-inventory-reversal.sql`. |
| **کار نیمه‌تمام (in-progress)** | ⚠️ باکس ۲ (`/order/checkout`) کد/tsc/build سبز ولی `db-ordering-checkout-migration.sql` هنوز روی DB production اجرا نشده — `/order/checkout` نمی‌تواند سفارش ثبت کند. باکس ۳ (`/orders` پنل پرسنل) کد/tsc/build کامل و سبز، migration جدیدش هم اجرا نشده (فقط هشدار صوتی Realtime را تحت‌تأثیر می‌گذارد، نه عملکرد اصلی). |
| **کار بعدی پیشنهادی** | (۱) کاربر هر دو migration را در pgAdmin/Supabase اجرا کند: `v0.9.9-order-checkout/db-ordering-checkout-migration.sql` (پیش‌نیاز `/order/checkout`) و `v0.9.10-order-board/db-ordering-board-realtime-migration.sql` (پیش‌نیاز هشدار صوتی `/orders`). (۲) تست end-to-end: ثبت سفارش از `/order/checkout` → ظهور فوری در `/orders` با هشدار → انتقال وضعیت‌ها تا تحویل/تکمیل + رهگیری مشتری + چاپ → BranchUser فقط شعبه‌ی خود. (۳) باکس بعدی سرویس سفارش: پرداخت آنلاین. (۴) دیپلوی `basharaf-tasks-ops-liara.zip` روی Liara (Backlog #14). (۵) retest باگ قدیمی «خطا در ثبت تجهیزات/سفارش خرید» (Backlog #15). |
| **بلاک‌شده/منتظر کاربر** | ⚠️ اجرای دو migration روی pgAdmin/Supabase: `v0.9.9-order-checkout/db-ordering-checkout-migration.sql` (بلاک‌کننده‌ی `/order/checkout`) و `v0.9.10-order-board/db-ordering-board-realtime-migration.sql` (فقط هشدار صوتی `/orders`). |
