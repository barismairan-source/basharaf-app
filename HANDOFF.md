...
| | |
|---|---|
| **نسخه** | `0.29.0` (روی branch `feat/notification-audience-control`؛ main هنوز `0.28.2`) |
| **آخرین به‌روزرسانی** | 2026-07-18 |
| **Build/tsc** | tsc ✅ ۰ خطا · tests 270/270 ✅ · build ✅ |
| **دیپلوی** | main = `0.28.2` (V2 + پنل موبایل‌فرندلی، هر دو روی production). یک فیچر بزرگ جدید («کنترل گیرندگان اعلان») کامل و تست‌شده روی branch جداست، هنوز **merge/deploy نشده** — عمداً، طبق دستور کاربر. |
| **کار نیمه‌تمام (in-progress)** | — |
| **کار بعدی پیشنهادی** | `project-docs/NOTIFICATION-AUDIENCE-CONTROL-ROLLOUT.md` بخوان → migration رو روی DB اجرا کن → branch رو به `main` merge کن → push (دیپلوی خودکار) → توی `/admin/settings/notifications` هر قانون رو باز کن و «گیرندگان» رو چک کن. |
