...
> ⚠️ **این سند قدیمی است — قبل از استفاده با کد فعلی تطبیق بده.** در نشست
> ۲۰۲۶-۰۷-۲۷ تأیید شد که چند گپ مهم توصیف‌شده در این سند (برگشت‌ناپذیری
> COGS/انبار هنگام حذف تراکنش، race condition تأیید تراکنش) از قبل رفع
> شده‌اند (`lib/db/balanceHelpers.ts` — `reverseSaleDeduction`+
> `voidCogsTransaction`؛ `SELECT FOR UPDATE` در `approve/route.ts`).
> جزئیات در `HANDOFF.md` بخش ۰.
