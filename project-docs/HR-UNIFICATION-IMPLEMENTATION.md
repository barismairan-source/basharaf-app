...
## فاز ۷ — نتیجه (تکمیل‌شده) + یک یافته‌ی مهم معماری

**یافته‌ی مهم (گزارش قبل از اقدام، طبق دستور):** موقع بررسی `PATCH /api/recruitment/[id]`، مشخص شد این route از قبل یک رفتار **مستند و عمدی** داشت: هر بار وضعیت به `accepted` تغییر می‌کرد، به‌طور ضمنی (idempotent با شماره‌تلفن، نه قفل واقعی) یک پرونده‌ی پرسنلی می‌ساخت. این دقیقاً همان چیزی بود که فاز ۷ باید با یک نسخه‌ی اتمیک/ردیابی‌شده جایگزین کند — ولی اگر هر دو مسیر (تغییر وضعیت به accepted + دکمه‌ی جدید «تبدیل به پرسنل») هم‌زمان زنده می‌ماندند، امکان **ساخت دو کارمند برای یک متقاضی** واقعی بود (چون مسیر قدیمی هیچ `sourceApplicationId`/`hiredAt` ثبت نمی‌کرد تا مسیر جدید تشخیص دهد قبلاً استخدام شده). **تصمیم:** رفتار ضمنی قدیمی از `PATCH` حذف شد (کامنت مفصل در همان فایل توضیح می‌دهد چرا)؛ از این به بعد «قبول‌کردن» (`status='accepted'`) فقط یک تصمیم است و «استخدام واقعی» فقط از مسیر اتمیک جدید ممکن است — این خودش دقیقاً همان تفکیک `accepted`/`hired` است که در دستور اصلی خواسته شده بود.

**تصمیم مهندسی دوم:** مقدار enum جدید `'hired'` (و `'offer'`) به `application_status` اضافه **نشد** — چون گسترش enum نیازمند تغییر هم‌زمان تب‌ها/شمارش‌ها/رنگ‌های کل UI کانبان استخدام بود (ریسک/گستره‌ی غیرضروری برای این فاز). به‌جایش: «استخدام واقعی» = `job_applications.hiredAt IS NOT NULL` (فیلد جدید، nullable، بدون تغییر enum). migration شامل دستور آماده (کامنت‌شده) برای افزودن این دو مقدار enum در آینده اگر لازم شد.

**عملیات اتمیک `POST /api/hr/recruitment/[id]/hire`:** درون یک `db.transaction`: قفل ردیف متقاضی (`SELECT ... FOR UPDATE`)، رد درخواست تکراری (چه با چک `hiredAt` چه با چک وجود `sourceApplicationId` در `employees`)، ساخت پرونده‌ی پرسنلی (نام/تلفن از متقاضی، `compensationType='hourly'` طبق تصمیم محصول، `sourceApplicationId` ست می‌شود)، به‌روزرسانی متقاضی (`status='accepted'`, `hiredAt`, `hiredBy`) — یا همه موفق یا هیچ‌کدام (خطا در وسط راه = rollback کامل، بدون رکورد نیمه‌کاره).

**اتصال داده:** `employees.sourceApplicationId` (nullable, یکتا با partial unique index) + `job_applications.hiredAt`/`hiredBy`. دکمه‌ی «تبدیل به پرسنل» در صفحه‌ی استخدام حالا این endpoint را صدا می‌زند (با تأیید قبل از اقدام) و بعد از موفقیت مستقیم به `/hr/people/{employeeId}` هدایت می‌کند (نه دیگر URL query hack قبلی). تب «منبع استخدام» در پرونده‌ی ۳۶۰ درجه حالا واقعی است (نام در درخواست، کانال آشنایی، امتیاز، تاریخ درخواست/استخدام).

**گزارش قیف استخدام:** `GET /api/hr/recruitment/funnel` (تعداد درخواست، نرخ ورود به لیست کوتاه، نرخ قبولی، نرخ استخدام واقعی، میانگین روز تا استخدام، کانال‌های جذب مؤثر، تفکیک استخدام به‌ازای سمت) + یک modal سبک در منوی «اقدامات بیشتر»ی صفحه‌ی استخدام (بدون دست‌زدن به چیدمان اصلی صفحه‌ی پیچیده‌ی موجود).

**محدودیت آگاهانه:** دکمه‌ی «تبدیل به پرسنل» در کارت/جزئیات داوطلب برای متقاضی از‌قبل‌استخدام‌شده غیرفعال/مخفی نشد (فقط با `confirm` + چک سمت سرور مسدود می‌شود) — تغییر ظاهری این دکمه نیازمند پاس‌دادن prop جدید از والد به دو کامپوننت (`CandidateCard`/`CandidateDetail`) بود که به فاز ۱۰ موکول شد.

**migration نوشته‌شده (اجرا نشده):** `db-hr-recruitment-employee-link.sql` — شامل کوئری تشخیص کارمندانی که با مکانیزم قدیمی (تطبیق تلفن) ساخته شده‌اند و هنوز `sourceApplicationId` ندارند، برای تصمیم دستی کاربر درباره‌ی backfill.

**گیت‌های این فاز:** tsc ✅ ۰ خطا · tests 548/548 ✅ (بدون تغییر — مسیر اتمیک DB-coupled است) · lint ✅ · build ✅ (`/api/hr/recruitment/[id]/hire`، `/api/hr/recruitment/funnel` در خروجی).

---

## فاز ۸ — نتیجه (تکمیل‌شده)

**اتصال:** `employees.userId` (nullable، یکتا با partial unique index) — پرسنل و کاربران سیستم در جدول‌های کاملاً جدا می‌مانند؛ این فقط یک لینک اختیاری است، نه ادغام. حذف کاربر یا غیرفعال‌کردن کارمند طرف مقابل را خودکار پاک نمی‌کند (`onDelete: set null`).

**API:** `POST`/`DELETE /api/employees/[id]/link-user` — فقط پشت `canDo(session, 'hr.systemAccess.manage')` (مدل دسترسی فاز ۲؛ پیش‌فرض فقط SuperAdmin). اتصال یک حساب که از قبل به کارمند دیگری وصل است رد می‌شود. هر دو عملیات با `audit()` ثبت می‌شوند (`hr.employee.userLinked`/`hr.employee.userUnlinked`، دو `AuditAction` جدید).

**تصمیم مهم — بدون تکرار منطق حساب کاربری:** «ساخت حساب جدید برای کارمند» یک endpoint مستقل نساخت؛ از همان `POST /api/users` موجود (که هش رمز عبور و اعتبارسنجی نقش/شعبه را از قبل درست انجام می‌دهد) استفاده می‌کند و بلافاصله بعد نتیجه را با endpoint جدید لینک می‌کند — پرهیز از دو نسخه از منطق حساسِ ساخت حساب.

**تب «حساب کاربری و دسترسی» (پرونده‌ی ۳۶۰ درجه):** حالا واقعی — وضعیت اتصال، نقش سیستم، وضعیت فعال/غیرفعال اگر متصل باشد؛ اگر نه، دو مسیر: «اتصال به حساب موجود» (انتخاب از فهرست کاربران) یا «ساخت حساب جدید» (فرم کامل نام/ایمیل/رمز/نقش/شعبه). سطح دسترسی نرم‌افزار (`role` کاربر) کاملاً مستقل از سمت شغلی (`role` کارمند) می‌ماند — دو مفهوم جدا، دقیقاً طبق دستور.

**گیت‌های این فاز:** tsc ✅ ۰ خطا · tests 548/548 ✅ (بدون تغییر — DB-coupled) · lint ✅ · build ✅ (`/api/employees/[id]/link-user` در خروجی).

---

## فاز ۹ — نتیجه (تکمیل‌شده)

**پنل آمادگی درون صفحه‌ی حقوق:** وقتی یک اجرا (`draft`/`calculated`) باز می‌شود، همان `GET /api/hr/payroll/readiness` فاز ۱ برای دوره/شعبه‌ی همان اجرا فچ می‌شود؛ اگر `criticalErrors` داشته باشد یک `InlineNotice` قرمز نشان می‌دهد (چون سرور هم دقیقاً همین‌ها را چک می‌کند، این پنل صرفاً پیش‌آگاهی زودتر است، نه تنها لایه‌ی جلوگیری — دکمه‌ی «محاسبه» واقعی سمت سرور همچنان رد می‌شود اگر خطای بحرانی باشد). اگر فقط `warnings` باشد، یک `InlineNotice` زرد جداگانه نشان می‌دهد.

**نوع محاسبه صریح در فهرست فیش‌ها:** `GET /api/payroll/runs/[id]` حالا `employees.compensationType` را join می‌کند و برمی‌گرداند؛ هر فیش یک Chip «ساعتی»/«ماهانه» صریح دارد. برای فیش‌های ساعتی، «تفکیک ساعتی» (نرخ‌های استفاده‌شده به‌ازای هر خط: عادی/اضافه‌کاری/شب‌کاری/تعطیل‌کاری/مرخصی‌باحقوق، با ساعت مربوطه از `meta.minutes`) هم از همان `calcSnapshot.lines` (که فاز ۱ در `calculateHourlyPayslip` می‌سازد) استخراج و نمایش داده می‌شود — بدون هیچ فرمول موازی جدید.

**محدودیت آگاهانه:** دکمه‌های «محاسبه»/«تأیید» در حالت جمع‌شده (قبل از باز‌کردن ردیف) غیرفعال نمی‌شوند — چون داده‌ی آمادگی فقط بعد از expand فچ می‌شود (برای جلوگیری از N فچ هم‌زمان برای هر ردیف در لیست). این یک مصالحه‌ی UX کم‌ریسک است، چون enforcement واقعی همیشه سمت سرور است؛ مدیر قبل از هر اقدام معمولاً ردیف را باز می‌کند تا فیش‌ها را ببیند.

**گیت‌های این فاز:** tsc ✅ ۰ خطا · tests 548/548 ✅ (بدون تغییر) · lint ✅ · build ✅.

---

## فاز ۱۰ — نتیجه (تکمیل‌شده، محدودشده به موارد کم‌ریسک/بلاتکلیف از فازهای قبل)

**حذف کامل `window.confirm`/`confirm()` از کل ماژول HR:** ۹ فراخوانی در ۶ فایل (`hr/people/page.tsx`، `hr/people/[id]/page.tsx`، `hr/payroll/page.tsx`، `hr/recruitment/page.tsx`، `hr/recruitment/form-builder/page.tsx`، `components/hr/ShiftScheduleView.tsx`، `components/hr/AttendanceView.tsx`) با `useConfirm()` موجود پروژه (`components/ui/ConfirmDialog.tsx`، از قبل در ریشه‌ی `(app)/layout.tsx` نصب‌شده) جایگزین شد — دقیقاً طبق دستور صریح «عدم استفاده از window.confirm»، بدون ساخت کامپوننت جدید (استفاده از چیزی که از قبل در سیستم طراحی پروژه بود).

**رفع محدودیت باقی‌مانده از فاز ۷:** دکمه‌ی «تبدیل به پرسنل» در `CandidateDetail` حالا وقتی `application.hiredAt` ست شده به‌جای دکمه یک نشان «استخدام شد ✓» نشان می‌دهد — بدون نیاز به prop جدید (چون `CandidateDetail` از قبل کل آبجکت application را داشت).

**محدودیت‌های آگاهانه‌ی باقی‌مانده (مستند، نه فراموش‌شده):**
1. **نمای هفتگی شبکه‌ای واقعی** برای تب برنامه شیفت (فاز ۶) ساخته نشد — نیازمند بازطراحی کامل `ShiftScheduleView` است (ردیف=کارمند، ستون=روز)؛ ریسک/گستره‌ی این کار برای باقی‌مانده‌ی این نشست بیش‌ازحد بود.
2. **یکسان‌سازی کامل container/padding** بین سه الگوی موجود (`PageShell` در استخدام، div دستی در پرسنل/حقوق، بدون‌پدینگ در زمان‌وحضور) انجام نشد — فاز ۳ عمداً این سه را دست‌نخورده نگه داشت تا پدینگ دوبرابر نشود؛ یکسان‌سازی واقعی نیازمند انتخاب یک الگوی واحد و migrate کردن هر سه صفحه به آن است.
3. صفحات هنوز `Skeleton`/حالت بارگذاری استاندارد یکسان ندارند (بعضی فقط متن «در حال بارگذاری…» دارند).

این سه مورد به‌عنوان کار دستی باقی‌مانده در گزارش نهایی فهرست می‌شوند، نه چیزی که ادعا شود انجام شده.

**گیت‌های این فاز:** tsc ✅ ۰ خطا · tests 548/548 ✅ (بدون تغییر — تغییرات این فاز صرفاً UI است) · lint ✅ (فقط ۲ warning preexisting در form-builder، بی‌ربط به این تغییرات) · build ✅.

---

## فاز ۱۱ — نتیجه (تکمیل‌شده)

هر ۵ migration در طول فازهای ۱/۵/۷/۸ همزمان با تغییر schema همان فاز نوشته شدند (نه یک‌جا در انتها) تا schema و migration هرگز از هم drift نکنند — دقیقاً الگوی فاز حقوق ساعتی نشست قبل. فایل جدید این فاز فقط سند rollout است: `project-docs/HR-UNIFICATION-ROLLOUT.md` — ترتیب اجرا، پیش/پس‌بررسی، مراحل deploy، smoke test، توضیح چرا اسکریپت backfill جدا با `HR_MIGRATION_CONFIRM` ساخته نشد (چون هیچ‌کدام از این ۵ migration داده‌ای را حذف/غیرقابل‌بازگشت تغییر نمی‌دهند)، روش تشخیص کارمند تکراری، روش اصلاح حضور ناسازگار، و فهرست تصمیم‌های باقی‌مانده‌ی مالک.

**هیچ migration در این نشست اجرا نشد.**

**گیت‌های این فاز:** بدون تغییر کد — فقط مستندسازی.
